...
test("landing presents Personal now and broader Aevric AI capabilities with truthful status",async({page})=>{
  await page.goto("/");
  await expect(page.getByText("Aevric AI OS",{exact:true})).toHaveCount(0);
  ...
  for(const heading of ["Organizations","Workspaces","Decision Center","Simulation Center","Marketplace","Billing","Current Intelligence","Aevric AI Models","Aevric AI Developer Platform","Aevric AI Research"]){
...
test("landing fluid layout fills common screens without overflow, overlap, or clipped cards",async({page})=>{
  ...
  for(const viewport of viewports){
    ...
    await expect(page.getByRole("link",{name:"Try Aevric AI"}).first()).toBeVisible();
    await expect(page.getByLabel("Personal Aevric AI conversation demonstration")).toBeVisible();
    ...
    const layout=await page.evaluate(()=>{
      ...
      const visibleElements=[...document.querySelectorAll("nav, main > section, main > footer, article, [aria-label='Personal Aevric AI conversation demonstration']")].filter(element=>{const style=getComputedStyle(element);const box=element.getBoundingClientRect();return style.display!=="none"&&box.width>0&&box.height>0;});
      return {
