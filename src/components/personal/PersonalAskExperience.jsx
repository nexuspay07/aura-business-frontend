...
function DecisionTurn({ decision }) {
  ...
  const resources = (decision.resources || []).map((item) => `${String(item.type || "resource").replaceAll("_", " ")}: ${item.value}`);
  const tensions = (decision.goal_tensions || []).map((item) => (item.between || []).join(" versus "));
  ...
  return <div className="mt-6 space-y-7 border-t border-white/10 pt-6">
    ...
    {decision.goals?.length > 0 && <section><h3 className="font-semibold text-white">Goals</h3>{list(decision.goals)}</section>}
    {tensions.length > 0 && <section><h3 className="font-semibold text-white">Competing goals</h3>{list(tensions)}</section>}
    {resources.length > 0 && <section><h3 className="font-semibold text-white">Resources & constraints</h3>{list(resources)}</section>}
    {(decision.uncertainties?.length > 0 || effects.length > 0) && <section><h3 className="font-semibold text-white">Risks & uncertainty</h3>{list([...(decision.uncertainties || []), ...effects])}</section>}
    ...
    {decision.prioritized_actions?.length > 0 && <section><h3 className="font-semibold text-white">Action plan</h3>{list(decision.prioritized_actions, actionText)}{decision.decision_plan?.style === "milestone" && <p className="mt-3 text-sm text-slate-400">Reassess at the midpoint and by the stated deadline.</p>}</section>}
  </div>;
}
...
function AuraTurn({ turn }) {
  ...
  const partial = turn.mode === "ANALYSIS_PARTIAL" ? turn.payload : null;
  ...
  return <article className={turn.role === "user" ? "ml-auto max-w-3xl rounded-3xl bg-blue-500 px-5 py-4 text-white sm:px-6" : "mr-auto max-w-4xl px-1 py-3 text-slate-200 sm:px-2"}>
    ...
    {decision && <DecisionTurn decision={decision}/>} {partial && <section className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[.06] p-5"><h3 className="font-semibold text-amber-100">Partial analysis</h3>{partial.goals?.length>0&&<><p className="mt-4 text-sm font-semibold text-slate-200">Goals</p>{list(partial.goals)}</>}{partial.key_facts?.length>0&&<><p className="mt-4 text-sm font-semibold text-slate-200">Facts retained</p>{list(partial.key_facts)}</>}</section>}
...
export default function PersonalAskExperience() {
  ...
  return <div className="mx-auto flex min-h-[calc(100vh-9rem)] w-full max-w-5xl flex-col px-1">
    ...
    {result?.mode === "ANALYSIS_PARTIAL" && <div className="mb-4 px-2"><button type="button" disabled={loading} onClick={() => setMessage(result.problem_understanding || "Please retry the analysis.")} className={`text-sm font-semibold text-blue-300 hover:text-blue-200 disabled:opacity-50 ${personal.focus}`}>Retry analysis</button></div>}
