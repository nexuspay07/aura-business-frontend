...
export default function PersonalAskExperience() {
  ...
  return <div className="mx-auto flex min-h-[calc(100vh-9rem)] w-full max-w-5xl flex-col px-1">
    ...
    <form onSubmit={submit} className="sticky bottom-0 rounded-[28px] border border-white/10 bg-[#0d121b]/95 p-3 shadow-2xl shadow-black/30 backdrop-blur sm:p-4"><label className="sr-only" htmlFor="ask-aura-message">Message Aevric AI</label><textarea id="ask-aura-message" value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={keyDown} disabled={loading || restoring} placeholder="Talk to Aevric AI..." rows={turns.length ? 3 : 6} className="w-full resize-none rounded-2xl border-0 bg-transparent px-4 py-3 text-lg leading-8 text-slate-100 outline-none placeholder:text-slate-600 focus:ring-1 focus:ring-blue-300/60 disabled:opacity-60 sm:px-5"/><div className="flex items-center justify-between gap-4 px-3 pb-1 pt-2"><p className="text-xs text-slate-500">Enter to send · Shift+Enter for a new line</p><button type="submit" disabled={!message.trim() || loading || restoring} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-500 px-5 py-2.5 font-semibold text-white transition hover:bg-blue-400 disabled:opacity-40 ${personal.focus}`}><Send size={17}/>{loading ? "Thinking…" : clarification ? "Continue" : "Send"}</button></div>{loading && <p role="status" className="mx-3 mt-2 flex items-center gap-2 border-t border-white/[.07] py-3 text-sm text-blue-200"><LoaderCircle className="animate-spin" size={16}/>Aevric AI is thinking…</p>}{error && <p role="alert" className="mx-3 mt-2 border-t border-white/[.07] py-3 text-sm text-red-200">{error}</p>}</form>
