import { BrainCircuit, Building2, LayoutDashboard, LogOut, PanelLeftClose, Sparkles, PanelsTopLeft, Library, FlaskConical, CreditCard, Settings } from "lucide-react";
...
export default function Sidebar({ mobileOpen = false, onClose = () => {} }) {
  ...
  return <>
    ...
    <aside aria-label="Primary navigation" className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-white/[.08] bg-[#0a0d13] text-slate-200 shadow-2xl transition-transform duration-200 lg:static lg:translate-x-0 lg:shadow-none ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="border-t border-white/[.08] p-5"><button onClick={signOut} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm text-slate-500 transition hover:bg-white/[.04] hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300"><LogOut size={18}/>Logout</button></div>
    </aside>
  </>;
}
